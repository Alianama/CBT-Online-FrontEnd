...
export default function ExamResults() {
  ...
  if(isError){
    ...
    <div className="flex flex-col items-center justify-center h-screen space-y-4 text-center">
      ...
      <h2 className="text-3xl font-bold text-red-600 mb-2">Terjadi Kesalahan</h2>
      <p className="text-lg text-gray-700 mb-4">Maaf, terjadi kesalahan saat memuat data. Silakan coba lagi nanti.</p>
      <Button onClick={() => navigate("/result")} className="bg-red-500 text-white hover:bg-red-600 transition-all duration-200">
        Kembali
      </Button>
    </div>
  ...
  if (!examData) {
    ...
    return (
      ...
      <div className="container mx-auto py-6 px-4 md:px-6 flex flex-col items-center justify-center animate-pulse">
        ...
        <h1 className="text-2xl font-bold mt-4 animate-blink">Memuat hasil ujian...</h1>
      </div>
    </Layout>
    )
  }
  ...
  return (
    ...
    <div className="container mx-auto py-6 px-4 md:px-6">
      <h1 className="text-2xl md:text-3xl font-bold mb-6">Hasil Ujian</h1>
      ...
      <div className="grid gap-6 md:grid-cols-3">
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              {[
                ...
                ["Jenis Ujian", info_ujian.jenis_ujian],
                ["Mata Pelajaran", info_ujian.nama_bank],
                ["Mulai", formatDate(info_ujian.mulai)],
                ["Selesai", formatDate(info_ujian.submit)],
              ].map(([label, value]) => (
        ...
        <Card className="md:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg">Hasil Ujian</CardTitle>
            <CardDescription>
              {info_ujian.nilai >= info_ujian.kkm
                ? "Selamat! Anda telah lulus ujian ini."
                : "Maaf, Anda belum lulus ujian ini."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Progress
                  value={(info_ujian.nilai / 100) * 100}
                  className="h-2"
                  />
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>0</span>
                  <span>KKM: {info_ujian.kkm}</span>
                  <span>100</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                {[
                  ...
                  <div
                  key={i}
                  className={`flex flex-col items-center p-3 rounded-lg ${item.bg}`}
                  >
                    {item.icon}
      ...
      <div className="mt-8">
        <h2 className="text-xl font-bold mb-4">
          Detail Soal ({totalQuestions})
        </h2>

        <Tabs defaultValue="all" className="w-full">
          <TabsList className="mb-4">
            <TabsTrigger value="all">Semua</TabsTrigger>
            <TabsTrigger value="correct">Benar</TabsTrigger>
            <TabsTrigger value="wrong">Salah</TabsTrigger>
          </TabsList>
