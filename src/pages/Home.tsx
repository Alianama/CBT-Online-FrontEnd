            ...
            <main className="min-h-screen bg-gradient-to-b from-background to-background/80">
                ...
                <div className="container mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                    ...
                    <section className="space-y-8">
                        ...
                        <ul className="space-y-6">
                            {instructionSteps[safeLocale].map((step, index) => (
                                <InstructionStep key={index} number={index + 1} title={step.title}
                    ...
                    <section className="space-y-6">
                        ...
                        <div
                            className="aspect-video w-full overflow-hidden rounded-xl border shadow-lg bg-muted relative group">
                            <iframe
                            ...
                            ></iframe>
                            <div
                                className="absolute inset-0 bg-primary/10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
                        </div>
...
function InstructionStep({number, title, description}: { number: number; title: string; description: string }) {
    return (
        <li className="relative pl-12 group">
            <div
                className="absolute left-0 top-0 flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-primary font-bold group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                {number}
            </div>
            ...
            <div className="space-y-2">
                <h3 className="text-xl font-medium group-hover:text-primary transition-colors flex items-center">
                    {title}
                    <ArrowRight
                        className="h-4 w-0 ml-2 opacity-0 group-hover:w-4 group-hover:opacity-100 transition-all"/>
                </h3>
...
const instructionSteps: Record<"id" | "en", { title: string; description: string }[]> = {
    id: [
        ...
        {title: "Mulai Ujian", description: "Tombol MULAI UJIAN hanya muncul selama durasi ujian berlangsung."},
        {
            ...
            description: "Siswa harus login sebelum ujian dimulai dan menyelesaikan tepat waktu."
        },
