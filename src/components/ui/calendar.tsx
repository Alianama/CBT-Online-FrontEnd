...
import { CalendarIndicator } from "@/components/schedule/calendar-indicator.tsx";
...
export function Calendar({ currentDate, events }: CalendarProps) {
  ...
  const getEventsForDate = (date: Date) => {
    return events?.filter((event) => {
      ...
      return (
        dateToCheck >= eventStart.setHours(0, 0, 0, 0) &&
        dateToCheck <= eventEnd.setHours(0, 0, 0, 0)
      );
    });
  };
  const isRangeBoundary = (
    ...
    const eventDate = new Date(
      type === "start" ? event.date_started : event.date_ended,
    );
