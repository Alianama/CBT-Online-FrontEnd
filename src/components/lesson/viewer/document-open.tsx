...
export default function DocumentOpen() {
    ...
    const pageData: Record<"id" | "en", { name: string; url: string }[]> = {
        id: [{name: "Materi", url: "/lesson"}, {name: "book", url: "#"}],
        en: [{name: "Lesson", url: "/lesson"}, {name: "Book", url: "#"}],
    };
    useEffect(() => {
        if (attachment) {
    ...
    const determinedFileType = tipe_materi?.toLowerCase() || file_url.split(".").pop()?.toLowerCase();
